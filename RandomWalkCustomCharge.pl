...
for ($i=($chains{'M'}+1); $i<=($chains{'M'}+$M); $i++) {
    $fi = 2*pi*rand();
    $Q = pi*rand();
    @randpos = ($Type, $b*sin($Q)*cos($fi),$b*sin($Q)*sin($fi),$b*cos($Q),$q);
        ...
        for (my $i=1; $i<=$chains{'M'}; $i++){
               for (my $k=1; $k<=$#{$chains{'mols'}[$i]}; $k++){
                ...
                @temp = @{$chains{'mols'}[$i][$k]};
                 printf FILE " $AtomNumber $MolNumber $chains{'mols'}[$i][$k][0] $chains{'mols'}[$i][$k][4] $chains{'mols'}[$i][$k][1] $chains{'mols'}[$i][$k][2] $chains{'mols'}[$i][$k][3]\n";
        }
        }
        printf FILE "\n";
...
my %sys = Polymatic::readLammps($_[0]);
...
###############   main  #############################
%chains=MakeWalk(2025,1,$b,$C,3,0);
AddWalk(\%chains,1,100,$b,$C,1,0.01);
AddWalk(\%chains,1,20,$b,$C,2,-0.01);
...
writeLammps(\%chains, 'chains_small.lmps');
